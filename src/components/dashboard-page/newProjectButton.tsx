...
export const NewProjectButton = component$<NewProjectButtonProps>((props) => {
  ...
  useVisibleTask$(async () => {
    OrganizationService.listOrganizations()
      .then((response: any) => {
        (response.data as OrganizationData[]).forEach((org) => {
          if (org.userRole === "admin") {
            isUserAdminOfOrganization.value = true;
            organization.value = org;
          }
        });
      })
      .catch((error: any) => {
        //AlertService.notifyError("Error getting organization!" + error);
      });
    ...
    const createNewProjectButton = document.querySelector(
      `#create-new-project-button-${props.variant}`
    )!;
  ...
  return (
    <div>
      {props.variant === 1 ? (
        <sl-button id="create-new-project-button-1" variant="primary">
          New Project
        </sl-button>
      ) : props.variant === 2 ? (
        <sl-button id="create-new-project-button-2" variant="primary" outline>
          Create a new project
        </sl-button>
      ) : (
        <sl-button id="create-new-project-button-3" variant="primary" outline>
          Create a new project
        </sl-button>
      )}
      <sl-dialog
                ...
                <div class="flex-column">
                  ...
                  {isUserAdminOfOrganization.value && (
                    <div class="create-org-proj">
                      ...
                      <div class="org-proj-description">
                        Organization Projects are accesible to your entire
                        Organization
                      </div>
                    </div>
                  )}
